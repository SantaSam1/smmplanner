...
const PLATFORMS = [
  ...
  { id: 'vk'       as Platform, label: 'ВКонтакте',      icon: '🔵', color: 'text-blue-700',   bg: 'bg-blue-50',   border: 'border-blue-200',  type: 'oauth'  as const, envKey: 'VITE_VK_APP_ID' },
...
const OAUTH_URLS: Record<string, { authUrl: string; scopes: string[]; sep: string }> = {
  ...
  vk:        { authUrl: 'https://oauth.vk.com/authorize', scopes: ['wall','photos','groups','offline'], sep: ',' },
};
...
export default function Accounts() {
  ...
  function handleOAuthReturn() {
    // VK implicit flow: token comes in URL hash
    if (window.location.hash.includes('access_token')) {
      const hash = new URLSearchParams(window.location.hash.slice(1));
      const vkToken = hash.get('access_token');
      ...
      if (vkToken && user) {
        ...
        supabase.from('social_accounts').insert({
          ...
          account_name: 'ВКонтакте',
          account_handle: userId || '',
          access_token: vkToken,
          is_active: true,
          followers_count: 0,
          credentials: { user_token: vkToken, token: vkToken },
        }).then(({ error }) => {
          if (!error) {
            setSuccessMsg(ru ? '✅ ВКонтакте подключён!' : '✅ VKontakte connected!');
